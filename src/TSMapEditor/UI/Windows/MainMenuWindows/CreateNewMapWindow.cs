...
namespace TSMapEditor.UI.Windows.MainMenuWindows
{
    ...
    public class CreateNewMapWindow : INItializableWindow
    {
        ...
        private void BtnCreate_LeftClick(object sender, EventArgs e)
        {
            ...
            {
                EditorMessageBox.Show(WindowManager, "地图太窄", "地图宽度必须至少 " + MinMapSize + " 格.", MessageBoxButtons.OK);
                return;
            }

            if (tbHeight.Value < MinMapSize)
            {
                EditorMessageBox.Show(WindowManager, "地图太小", "地图高度必须至少 " + MinMapSize + " 格.", MessageBoxButtons.OK);
                return;
            }

            if (tbWidth.Value > Constants.MaxMapWidth)
            {
                EditorMessageBox.Show(WindowManager, "地图太宽", "地图宽度不能超过 " + Constants.MaxMapWidth + " 格.", MessageBoxButtons.OK);
                return;
            }

            if (tbHeight.Value > Constants.MaxMapHeight)
            {
                EditorMessageBox.Show(WindowManager, "地图太长", "地图高度不能超过 " + Constants.MaxMapHeight + " 格.", MessageBoxButtons.OK);
                return;
            }

            if (tbWidth.Value + tbHeight.Value > MaxMapSize)
            {
                EditorMessageBox.Show(WindowManager, "地图太大", "地图宽度+高度不能超过。 " + MaxMapSize + " 格.", MessageBoxButtons.OK);
                return;
            }
